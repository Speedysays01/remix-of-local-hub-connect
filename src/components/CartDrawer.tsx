...
const CartDrawer: React.FC = () => {
  ...
  // Confirmation state: null = closed, string = cartItemId being confirmed
  const [confirmRemoveId, setConfirmRemoveId] = useState<string | null>(null);

  const confirmingItem = items.find((i) => i.id === confirmRemoveId);
  ...
  // Decrement — if qty would reach 0, ask for confirmation first
      ...
      <AlertDialog open={!!confirmRemoveId} onOpenChange={(o) => !o && setConfirmRemoveId(null)}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              {confirmingItem
                ? <>Remove <strong>{confirmingItem.product.name}</strong> from your cart?</>
